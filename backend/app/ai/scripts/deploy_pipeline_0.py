"""
Script pour créer un deployment du Pipeline analyze_intent sur CraftAI
"""
...
def main():
    print("=" * 80)
    print("Déploiement du Pipeline - analyze_intent")
    print("=" * 80)
    ...
    deployment_name = "analyze-intent-deploy"
    ...
    try:
        ...
        deployment = sdk.create_deployment(
            ...
            description="Deployment du pipeline d'analyse d'intention utilisateur"
        )
        ...
        if 'endpoint_token' in deployment:
            print(f"   Endpoint Token: {deployment.get('endpoint_token')[:20]}...")
        ...
        print(f"        -d '{{\"message\": \"Votre message ici\"}}'")

    except Exception as e:
        print(f"\n❌ Erreur lors du déploiement: {e}")
        print(f"\nℹ️  Vérifiez:")
        print(f"   - Que le pipeline 'analyze-intent' existe")
