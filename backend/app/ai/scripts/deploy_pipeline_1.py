"""
Script pour créer un deployment du Pipeline extract_legifrance sur CraftAI
"""
...
def main():
    print("=" * 80)
    print("Déploiement du Pipeline - extract-legifrance")
    print("=" * 80)
    ...
    if not os.getenv("MIBS_LEGIFRANCE_CLIENT_ID"):
        print("❌ Erreur: MIBS_LEGIFRANCE_CLIENT_ID n'est pas définie")
        return

    if not os.getenv("MIBS_LEGIFRANCE_CLIENT_SECRET"):
        print("❌ Erreur: MIBS_LEGIFRANCE_CLIENT_SECRET n'est pas définie")
        return

    sdk = CraftAiSdk()

    deployment_name = "extract-legifrance-deploy"
    ...
    environment_variables = {
        ...
        "MIBS_LEGIFRANCE_CLIENT_ID": os.getenv("MIBS_LEGIFRANCE_CLIENT_ID"),
        "MIBS_LEGIFRANCE_CLIENT_SECRET": os.getenv("MIBS_LEGIFRANCE_CLIENT_SECRET"),
        "MIBS_LEGIFRANCE_TOKEN_URL": os.getenv("MIBS_LEGIFRANCE_TOKEN_URL", "https://sandbox-oauth.piste.gouv.fr/api/oauth/token"),
        "MIBS_LEGIFRANCE_API_URL": os.getenv("MIBS_LEGIFRANCE_API_URL", "https://sandbox-api.piste.gouv.fr/dila/legifrance/lf-engine-app")
    }
    ...
    try:
        ...
        deployment = sdk.create_deployment(
            ...
            description="Deployment du pipeline d'extraction de textes juridiques depuis Légifrance"
        )
        ...
        print(f"   PIPELINE_1_ENDPOINT_URL={deployment.get('endpoint_url', 'ENDPOINT_URL')}")
        print(f"   PIPELINE_1_ENDPOINT_TOKEN={deployment.get('endpoint_token', 'ENDPOINT_TOKEN')}")
        ...
        print(f"        -d '{{\"message\": \"Quelles sont les conséquences de la dissolution d'un PACS ?\", \"intention\": \"DEBAT\"}}'")

    except Exception as e:
        print(f"\n❌ Erreur lors du déploiement: {e}")
        print(f"\nℹ️  Vérifiez:")
        print(f"   - Que le pipeline 'extract-legifrance' existe")
